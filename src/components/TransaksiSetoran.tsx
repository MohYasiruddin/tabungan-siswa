...
import { Setoran, Siswa } from '../types';
...
interface SetoranProps {
  ...
  setoranList: any[];
  loading: boolean;
  onAddSetoran: (data: any) => Promise<boolean>;
  onRefresh: () => void;
  petugasName: string;
}

export default function TransaksiSetoran({ 
  siswaList, 
  setoranList, 
  loading, 
  onAddSetoran,
  ...
  const [keterangan, setKeterangan] = useState('Setoran Tabungan');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(value) || value <= 0) {
      setFormError('Nominal setoran harus berupa angka positif!');
      return;
    }
    ...
    try {
      ...
      const isSuccess = await onAddSetoran(data);
      if (isSuccess) {
        setFormSuccess('Setoran tabungan berhasil dicatat!');
        
        // Auto-open print receipt with metadata
        setShowReceipt({
          id_transaksi: 'TXS-' + Date.now(),
        ...
        setNominal('');
        setKeterangan('Setoran Tabungan');
      } else {
        setFormError('Gagal menambahkan transaksi setoran ke database.');
      }
    } catch (err: any) {
...
Kami menginformasikan bahwa transaksi *Setoran Tabungan* siswa di *AL-FUNGSI* telah berhasil dibukukan dengan rincian sebagai berikut:

👤 *Nama Siswa:* ${studentName} (${studentNis})
🏫 *Kelas:* Class ${studentClass}
📅 *Tanggal:* ${dateFormatted}
💵 *Nominal Setor:* *${nominalFormatted}*
📊 *Saldo Akhir:* *${currentBalance}*
📝 *Keterangan:* ${receipt.keterangan || 'Setoran Tabungan'}
...
*Layanan Keuangan Sekolah AL-FUNGSI*`;
  ...
  // Filter history list
  const filteredLedger = setoranList.filter(s => {
    const sNama = s.siswa_nama ? String(s.siswa_nama).toLowerCase() : '';
    const sNis = s.siswa_nis ? String(s.siswa_nis) : '';
    const sId = (s.id_transaksi || s.id_setoran) ? String(s.id_transaksi || s.id_setoran) : '';
    const sKet = s.keterangan ? String(s.keterangan).toLowerCase() : '';
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="space-y-6 lg:col-span-1">
        <div className="p-6 bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-805 shadow-xs flex flex-col justify-between">
          <div className="mb-5 flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-sm text-zinc-900 dark:text-white uppercase tracking-wider">Input Setoran Kas</h3>
              <p className="text-[10px] text-zinc-400">Pencatatan kas masuk tabungan siswa</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {!selectedSiswa ? (
                <>
                  ...
                  {searchStudent.trim().length > 0 && (
                    <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-zinc-900 rounded-xl shadow-xl border border-zinc-200 dark:border-zinc-800 max-h-48 overflow-y-auto z-10 p-1.5 divide-y divide-zinc-100 dark:divide-zinc-850">
                      {lookupSiswa.length > 0 ? (
                        lookupSiswa.map(s => (
                          ...
                          >
                            <div>
                              <span className="font-bold text-zinc-800 dark:text-zinc-100 block">{s.nama}</span>
                              <span className="text-[10px] text-zinc-400 mt-0.5 font-mono">{s.nis} • Class {s.kelas}</span>
                            </div>
                            <span className="text-[10px] text-blue-600 font-bold font-mono bg-blue-50 dark:bg-zinc-950 px-2 py-0.5 rounded-md border border-blue-100">
                              {rupiah(s.saldo)}
                            </span>
                          </button>
                        ))
                ...
                <div className="p-3.5 bg-zinc-950 text-white rounded-xl border border-zinc-850 flex justify-between items-center">
                  <div>
                    <span className="font-bold text-xs block text-blue-400">{selectedSiswa.nama}</span>
                    <span className="text-[10px] text-zinc-400 block mt-0.5 font-mono">NIS: {selectedSiswa.nis} • {selectedSiswa.kelas}</span>
                    <span className="text-[10px] text-zinc-500 block mt-0.5">Saldo Saat Ini: <b>{rupiah(selectedSiswa.saldo)}</b></span>
                  </div>
                  <button
                    type="button"
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-zinc-500 dark:text-zinc-400 block uppercase">Nominal Setoran (RP)</label>
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full py-2.5 px-4 rounded-lg text-xs font-semibold bg-blue-600 hover:bg-blue-700 text-white flex justify-center items-center gap-2 cursor-pointer shadow-xs transition-colors disabled:opacity-50"
            >
              {submitting ? (
      ...
      {/* Ledger history column (Right) */}
      <div className="lg:col-span-2 space-y-4">
        
        {/* Statistics list banner */}
        <div className="p-4 bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-805 shadow-xs flex flex-col md:flex-row gap-4 justify-between items-center">
          ...
          <div className="relative w-full md:max-w-xs flex items-center bg-zinc-50 dark:bg-zinc-950 px-3 py-2 rounded-xl border border-zinc-200 dark:border-zinc-850">
            ...
            <input
              type="text"
              placeholder="Cari riwayat setoran..."
              value={searchLedger}
              onChange={(e) => {
          ...
          <div className="text-right text-xs text-zinc-400 inline-flex items-center gap-1.5">
            ...
            <span>Terekam {filteredLedger.length} Transaksi Setoran</span>
          </div>
        </div>

        {/* List of Setoran transaction table */}
        <div className="overflow-hidden bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-xs">
            ...
            <tbody className="divide-y divide-zinc-150 dark:divide-zinc-805 text-zinc-700 dark:text-zinc-300 text-xs font-medium">
              {currentItems.length > 0 ? (
                currentItems.map((s: any) => (
                  <tr key={s.id_transaksi || s.id_setoran} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/40 transition-colors">
                    <td className="px-6 py-3.5">
                      <div className="font-semibold text-zinc-905 dark:text-white">{s.siswa_nama}</div>
                      <div className="text-[10px] text-zinc-400 mt-0.5 font-mono">{s.siswa_nis} • Class {s.siswa_kelas}</div>
                    </td>
                    <td className="px-6 py-3.5 font-mono text-[11px] text-zinc-500">{s.tanggal}</td>
                    <td className="px-6 py-3.5 font-semibold font-mono text-blue-600 dark:text-blue-400">{rupiah(s.nominal)}</td>
                    <td className="px-6 py-3.5 text-zinc-500 truncate max-w-[150px]" title={s.keterangan}>{s.keterangan || 'Setoran Tabungan'}</td>
                    <td className="px-6 py-3.5 text-right whitespace-nowrap">
                      <button
                        onClick={() => setShowReceipt(s)}
                        className="p-1.5 rounded-lg border border-zinc-200 dark:border-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-650 dark:text-zinc-300 transition-colors inline-flex items-center gap-1.5 cursor-pointer text-[11px] font-bold"
                      >
                ...
                <tr>
                  <td colSpan={5} className="py-12 text-center text-zinc-450">
                    <Coins className="w-8 h-8 mx-auto text-zinc-300 stroke-1 mb-2" />
                    <span className="text-xs">Umpan data setoran kosong.</span>
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {/* Ticket / Invoice Receipt Dialog Modal */}
      {showReceipt && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
          <div className="bg-white dark:bg-zinc-900 rounded-2xl w-full max-w-sm shadow-2xl border border-zinc-200 dark:border-zinc-800 flex flex-col justify-between overflow-hidden">
            ...
            <div className="p-4 bg-zinc-50 dark:bg-zinc-950/40 border-b border-zinc-150 dark:border-zinc-800 flex justify-between items-center">
              <span className="text-xs font-bold text-zinc-500 uppercase tracking-widest font-mono">Karcis Bukti Setoran</span>
              <button 
                onClick={() => setShowReceipt(null)}
                className="p-1.5 rounded-lg border border-zinc-250 dark:border-zinc-800 hover:bg-zinc-150 dark:hover:bg-zinc-800 transition-colors cursor-pointer"
              >
            ...
            {/* Print Body Block */}
            <div id="receipt-print-area" className="p-6 bg-white dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 font-mono text-xs space-y-4">
              ...
              <div className="text-center pb-3 border-b border-dashed border-zinc-300 dark:border-zinc-700 space-y-1">
                <span className="font-bold text-sm block tracking-tighter uppercase">KOP SEKOLAH AL-FUNGSI</span>
                <p className="text-[10px] text-zinc-400">Bukti Setor Kas Tabungan Resmi</p>
                <span className="text-[9px] text-zinc-500 block">ID: {showReceipt.id_setoran || showReceipt.id_transaksi}</span>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between">
                  <span className="text-zinc-400">Tanggal Setor:</span>
              ...
              <div className="py-2.5 bg-zinc-50 dark:bg-zinc-950 px-3 rounded-lg border border-dashed border-zinc-150 dark:border-zinc-800 flex justify-between items-center text-sm font-bold">
                <span className="text-zinc-500 text-xs">NOMINAL:</span>
                <span className="text-blue-605 dark:text-blue-400 font-bold">{rupiah(showReceipt.nominal)}</span>
              </div>
              ...
              {/* Warning footer */}
              <div className="text-center pt-2 text-[10px] text-zinc-400 border-t border-dashed border-zinc-300 dark:border-zinc-700 leading-relaxed">
                <p>Simpan tanda bukti transaksi ini seaman mungkin sebagai rekonsiliasi bulanan.</p>
            ...
            <div className="p-4 bg-zinc-50 dark:bg-zinc-950 border-t border-zinc-150 dark:border-zinc-800 flex flex-col gap-2.5">
              ...
              <div className="flex gap-2 w-full">
                <button
                  onClick={() => setShowReceipt(null)}
                  className="w-1/2 py-2 text-xs font-semibold bg-zinc-200 hover:bg-zinc-305 dark:bg-zinc-850 dark:hover:bg-zinc-800 text-zinc-800 dark:text-zinc-250 rounded-lg text-center cursor-pointer transition-colors"
                >
                  Tutup
                </button>
                <button
                  onClick={handlePrintReceipt}
                  className="w-1/2 py-2 text-xs font-semibold bg-blue-600 hover:bg-blue-700 text-white rounded-lg flex justify-center items-center gap-1.5 cursor-pointer shadow-xs transition-colors"
                >
                  <Printer className="w-3.5 h-3.5" />
